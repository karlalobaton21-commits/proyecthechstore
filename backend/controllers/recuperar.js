...
export const solicitarCodigo = async (req, res) => {
    try {
        ...
        usuario.codigoExpiracion = new Date (Date.now() + 900000); //15 minutos
        await usuario.save();

        // 🔍 VERIFICAR QUE SE GUARDÓ CORRECTAMENTE
        //const verificar = await Users.findOne({Correo});
        //console.log('💾 Código guardado en DB:', verificar.codigoRecuperacion, 'Tipo:', typeof verificar.codigoRecuperacion);
...
export const cambiarPassword = async (req, res) => {
    try{
        const { Correo, codigo, nuevaPassword, confirmPassword } = req.body;

        //validacion
        if (!Correo || !codigo || !nuevaPassword || !confirmPassword) {
            return res.status(400).json({
...
//idk
        if (!usuario) {
            return res.status(400).json({
                 message: "Código de recuperación inválido, expirado o el correo no coincide."
            });
        }
